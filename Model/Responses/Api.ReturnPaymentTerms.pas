...
constructor TReturnPaymentTerms.Create(Count: cardinal = 1);
begin

    if not Assigned(FPaymentTerms) then
    begin
        SetLength(FPaymentTerms, Count);
        for var Index:=0 to Count - 1 do FPaymentTerms[Index]:=TPaymentTermsFields.Create();
    end;
